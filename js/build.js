          ...
          return Fliplet.Hooks.run('afterQueryChart', result)
            ...
            .then(function() {
              ...
              switch (data.dataSourceQuery.selectedModeIdx) {
                  ...
                  result.dataSourceEntries.forEach(function(row, i) {
                    data.entries.push({
                      name:
                        ...
                        T('widgets.charts.donut.category', { count: i + 1 }),
                      y:
                        parseInt(row[data.dataSourceQuery.columns.value], 10) ||
                        0
                    });
                  });
                  break;
                case 1:
                  // Summarize data
                  data.name = T('widgets.charts.donut.title', {
            ...
            .then(function() {
              data.entries = _.reverse(
                _.sortBy(data.entries, function(o) {
                  return o.y;
                })
              );

              // SAVES THE TOTAL NUMBER OF ROW/ENTRIES
              data.totalEntries = _.reduce(
                data.entries,
                function(sum, o) {
                  return sum + o.y;
                },
